...
export class InstagramPhotosCard extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (profile) {
      let response = await fetch(`${profile}?__a=1`);
...
export class MapCard extends React.Component {
  ...
  render() {
    let { address, city, postalCode, name } = this.props.outfit;
  ...
  _maybeRenderOverlay() {
    ...
    if (this.state.shouldRenderMap) {
      ...
      return <View style={[styles.map, { backgroundColor: '#597172' }]} />;
    }
  }
  ...
  _maybeRenderMap() {
    ...
    let { name, latitude, longitude } = this.props.outfit;
...
export class VisitedCard extends React.Component {
  static getDataProps(data, props) {
    let { id: outfitId } = props.outfit;
    let isVisited = data.outfits.visited.includes(outfitId);
    console.log("I guess these are visited: " + JSON.stringify(data.outfits.visited) + " and this is the id: " + outfitId);
  ...
  _onToggleVisited = () => {
    if (this.props.isVisited) {
      this.props.dispatch(Actions.removeVisitedOutfit(this.props.outfit.id));
      console.log("Trying to get watch id state. " + this.state);
      if (this.state["watchId"]) {
        navigator.geolocation.clearWatch(this.state["watchId"]);
        this.setState({watchId: null});
      }
    } else {
      this.props.dispatch(Actions.addVisitedOutfit(this.props.outfit.id));
      let watchId = navigator.geolocation.watchPosition(
          (position) => {
            let {latitude, longitude} = this.props.outfit;
            console.log("The outfit: " + JSON.stringify(this.props.outfit) + "lat: " + latitude + " lng: " + longitude + " as compared to\n\n " + JSON.stringify(position));
            /* success */
            ...
            console.log("The watch has seen " + this.props.outfit.id + " is " + distanceM + " meters away.");
            if (distanceM < 100) {

import transformOutfit from '../transformOutfit';

describe('hours', () => {
  it('correctly calculates when open', () => {
    const openDateTime = new Date("Fri Oct 21 2016 15:55:00 GMT-0700 (PDT)");
    const outfit = transformOutfit(exampleOutfit, openDateTime);
    expect(outfit.isOpen).toBe(true);
  });

  it('correctly calculates when closed', () => {
    const closedDateTime = new Date("Fri Oct 21 2016 23:55:00 GMT-0700 (PDT)");
    const outfit = transformOutfit(exampleOutfit, closedDateTime);
    expect(outfit.isOpen).toBe(false);
  });
});
